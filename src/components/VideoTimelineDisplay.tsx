...
interface VideoTimelineDisplayProps {
  videoUrl: string;
  duration: number;
  currentTime: number;
...
export const VideoTimelineDisplay = ({
  duration,
  currentTime,
  splitPoints,
  onAddSplitPoint,
  onUpdateSplitPoint,
}: VideoTimelineDisplayProps) => {
  ...
  useEffect(() => {
    ...
    drawTimeline();
  }, [currentTime, splitPoints, hoveredPointId, dragState, duration]);
    ...
    <Card className="bg-waveform-bg border-border/30 p-4">
      <div className="space-y-4">
        <div className="text-sm text-muted-foreground">
          {isLoading ? 'Video wordt geladen...' : 'Klik op de timeline om splitpunten toe te voegen'}
        </div>
        
        <div ref={containerRef} className="w-full">
          {isLoading ? (
            <div className="relative w-full h-[100px] rounded border border-border/20 bg-secondary/20 overflow-hidden">
